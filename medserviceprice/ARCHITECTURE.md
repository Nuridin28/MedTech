...
## 6. Поиск, сравнение, врачи, AI-ассистент
...
- **Приём врача:** live-прокси doq (`services/doq_live.py`) — список врачей с ценой/рейтингом/специальностью, **живые слоты** записи, фильтр **«удобное время»** (день недели + диапазон часов, батч-проверка слотов), сортировки рейтинг/время/цена/расстояние/опыт.
...
|--------|----------|-----|
| 3 веб-источника + файлы, пайплайн+дедуп, нормализация (+AI), поиск, фильтры, сортировки, карта, сравнение, чек-ап, **врачи+слоты (doq live)**, история, подписки+рассылка, AI-ассистент (tool-calling), обогащение, админ-SPA, ELK (опц. профиль), алёрты, i18n, PWA, **тесты+CI**, Docker | рейтинги/отзывы (ключ 2GIS), AI-генерация каталога (ключ OpenAI) | бэкапы Postgres, метрики Prometheus, дедуп клиник между источниками, SEO-лендинги |

**Тесты/CI:** unit-тесты ядра (нормализация, дедуп-хэши) — `backend/tests/`, запуск `python run_tests.py` или `pytest`. GitHub Actions (`.github/workflows/ci.yml`) гоняет бэкенд-тесты + `tsc && vite build` на каждый push/PR.
