...
export function SearchResultsPage() {
  ...
  const serviceName = items[0]?.service_name_norm ?? q ?? 'Medical service'
    ...
    <main className="max-w-container-max mx-auto px-margin-mobile lg:px-margin-desktop py-6 lg:py-8">
      ...
      <header className="mb-6 lg:mb-10">
        <div className="flex items-center gap-2 text-on-surface-variant mb-1">
          ...
          <span className="font-label-bold">{city ?? 'All cities'}</span>
        </div>
        ...
        <p className="text-text-subtle font-body-md">
          {isLoading ? 'Searching…' : `${total} ${total === 1 ? 'offer' : 'offers'} found`}
        </p>
        ...
        {data && total > 0 && (
          <div className="mt-4 grid grid-cols-3 gap-3 bg-surface-container-lowest dark:bg-surface-container border border-outline-variant rounded-xl p-4 max-w-xl">
            <div>
              <p className="text-text-subtle font-body-sm mb-0.5">Lowest</p>
              <p className="font-price-display text-success-green">{formatPrice(data.price_min)}</p>
            </div>
            <div>
              <p className="text-text-subtle font-body-sm mb-0.5">Average</p>
              <p className="font-price-display text-on-surface">{formatPrice(data.price_avg)}</p>
            </div>
            <div>
              <p className="text-text-subtle font-body-sm mb-0.5">Highest</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-gutter">
        ...
        >
          <div className="bg-surface-container-lowest dark:bg-surface-container border border-outline-variant p-6 rounded-xl filter-transition">
            <div className="flex items-center justify-between mb-6">
              <h3 className="font-label-bold text-on-surface">Filters</h3>
              <button
                type="button"
                onClick={() =>
                  patchParams({
            ...
            {/* Price range */}
            <div className="mb-8">
              <label className="block font-label-bold mb-4">Price Range (₸)</label>
        ...
        <div className="lg:col-span-9">
          ...
          <div className="hidden lg:flex bg-surface-container-lowest dark:bg-surface-container border border-outline-variant p-4 rounded-xl mb-6 flex-wrap items-center gap-4">
            <span className="font-label-bold text-text-subtle">Sort by:</span>
          ...
          {!isLoading && isError && (
            <EmptyState
              ...
              description="Something went wrong while fetching prices. Please try again in a moment."
              action={
          ...
          {view !== 'compare' && !isLoading && !isError && items.length === 0 && (
            <EmptyState
              ...
              description="Try adjusting your filters or searching for a different service."
              action={
                <Link
                  to="/"
