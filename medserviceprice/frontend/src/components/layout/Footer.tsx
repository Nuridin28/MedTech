      ...
      <div className="w-full py-12 px-margin-mobile md:px-margin-desktop max-w-container-max mx-auto flex flex-col md:flex-row justify-between gap-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-12">
          <FooterCol
            title={t('Platform')}
            links={[
              { label: t('Find prices'), to: '/search' },
              { label: t('For Clinics'), to: '/dashboard' },
              { label: t('Saved clinics'), to: '/favorites' },
            ]}
          />
          <FooterCol
            title={t('Account')}
            links={[
              ...
              { label: t('Appointments'), to: '/appointments' },
              { label: t('Medical records'), to: '/records' },
            ]}
          />
          <div className="col-span-2 md:col-span-1">
            <div className="font-label-bold text-label-bold text-primary mb-4">{t('Follow Us')}</div>
            <div className="flex gap-4 text-on-surface-variant dark:text-surface-variant">
              <a href="#" aria-label={t('Telegram')} className="hover:text-primary transition-colors">
                <Icon name="send" />
              </a>
              <a href="#" aria-label={t('Website')} className="hover:text-primary transition-colors">
                <Icon name="language" />
              </a>
            </div>
          </div>
